...
description: "A step-by-step guide on how to use the Product Review Card component in your project. you can use this product card component if you are using Nextjs or Reactjs with Typescript and Tailwind css for a project."
---

### Review

<Frame>
  <img src="/images/review.png" style={{ borderRadius: "0.5rem" }} />
</Frame>

## Make a component

make a component file ProductReview.tsx and paste the below code:
...
const ReviewCard: React.FC<CardProps> = ({ id, username, rating, description }) => {
  ...
  const displayText = shouldTruncate && !isExpanded ? description.slice(0, 250) + "..." : description;
...
```jsx
import Rating from "@your-path/ReviewCard";

export default function Example() {
  return (
    <ReviewCard
      id={"abcd123"}
      username={"swapnilhkhk"}
      rating={3}
      description={"it is a good bottle for gym and all."}
    />
  );
}
```
