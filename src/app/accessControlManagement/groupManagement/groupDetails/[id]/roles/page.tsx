...
import { useRolesData, Role } from "../../../createGroup/useRolesData";
import DynamicTable from "@/components/ui/DynamicSessionTable";

const columns: { key: keyof Role; header: string }[] = [
...
export default function AddRoles({
  ...
  useEffect(() => {
    params.then((resolvedParams) => {
      if (resolvedParams?.id) {
        const extractedGroupId = decodeURIComponent(resolvedParams.id);
  ...
  // Handle navigation for details button
  ...
  const handleRoleSelectionChange = (selectedIds: string[]) => {
    setSelectedRoleIds(selectedIds);
  };

  const handleSaveRoles = async () => {
    if (!groupId || selectedRoleIds.length === 0) {
      alert("No roles selected or invalid group ID.");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/fitnearn/web/admin/access/group/add-role`,
        {
          method: "PATCH",
          headers: {
      ...
      if (response.ok && result.success) {
        ...
        alert("Failed to add roles: " + result.message);
      }
    } catch (error) {
      console.error("Error adding roles:", error);
      alert("An error occurred while adding roles.");
    }
  };
...
>
  <Header isOpen={isOpen} setIsOpen={setIsOpen}  />
      <LiveSessionForm
        breadcrumb={["Access Control Management", "Create Group", "Add Roles"]}
        breadcrumbLinks={[
        ...
        onActionButtonClick={handleSaveRoles}
        actionButtonIcon={
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
