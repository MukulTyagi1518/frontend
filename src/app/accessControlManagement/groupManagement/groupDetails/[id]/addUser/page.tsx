...
import { useAddUser, User } from "../../../createGroup/useAddUser";
...
export default function AddUsers({
  ...
  const handleSaveUsers = async () => {
    if (!groupId || selectedUserIds.length === 0) {
      alert("No users selected or invalid group ID.");
      return;
    }

    const requestBody = {
      groupId: groupId, // ✅ Use extracted groupId
      userId: selectedUserIds, // Send selected user IDs
    };

    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/fitnearn/web/admin/access/group/add-user`,
        {
          method: "PATCH",
          headers: {
      ...
      if (response.ok && result.success) {
        ...
        alert("Failed to add users: " + result.message);
      }
    } catch (error) {
...
>
  <Header isOpen={isOpen} setIsOpen={setIsOpen}  />
      <LiveSessionForm
        breadcrumb={["Access Control Management", "Group Details", "Add Users"]}
        breadcrumbLinks={[
          "/accessControlManagement/adminUser/table",
          `/accessControlManagement/groupManagement/groupDetails/${groupId}`,
          `/accessControlManagement/groupManagement/groupDetails/${groupId}/addUsers`,
        ]}
        breadcrumbIcons={[
          <svg
        ...
        onActionButtonClick={handleSaveUsers}
        actionButtonIcon={
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
