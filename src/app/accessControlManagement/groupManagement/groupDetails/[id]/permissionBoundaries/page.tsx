...
import {
  ...
  Permission,
} from "../../../createGroup/usePermissionBoundaries";
...
export default function AddPermissionBoundary({
  params,
}: {
  ...
  const [selectedPermissionBoundaryIds, setSelectedPermissionBoundaryIds] =
    useState<string[]>([]);
  ...
  const handlePermissionBoundarySelectionChange = (selectedIds: string[]) => {
    setSelectedPermissionBoundaryIds(selectedIds);
  };

  const handleSavePermissionBoundaries = async () => {
    if (!groupId || selectedPermissionBoundaryIds.length === 0) {
      alert("No permission boundaries selected or invalid group ID.");
      return;
    }

    const requestBody = {
      groupId: groupId,
      permissionBoundaryId: selectedPermissionBoundaryIds,
    };

    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/fitnearn/web/admin/access/group/add-permission-boundary`,
        {
          method: "PATCH",
          headers: {
      ...
      if (response.ok && result.success) {
        alert("Permission Boundaries added successfully!");
      } else {
        alert("Failed to add permission boundaries: " + result.message);
      }
    } catch (error) {
      console.error("Error adding permission boundaries:", error);
      alert("An error occurred while adding permission boundaries.");
    }
  };
...
>
  <Header isOpen={isOpen} setIsOpen={setIsOpen}  />
      <LiveSessionForm
        ...
        onActionButtonClick={handleSavePermissionBoundaries}
        actionButtonIcon={
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
      ...
      >
        <DynamicTable
          ...
          onSelectionChange={handlePermissionBoundarySelectionChange}
        />
      </LiveSessionForm>
      </div>
    </>
